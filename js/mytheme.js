...
$(document).ready(function() {
	...
	setTimeout(function() {
		$(".home-tagline").animate({opacity: 1});
	}, 2500); 
	setTimeout(function() {
		$("#home-scroll, .home.page #header").animate({opacity: 1});
	}, 3000); 
...
$(window).scroll(function() {
  if ($(window).width() > 999) {
    ...
    if (sT >= 10) {
      ...
      $('#nav-toggle span').addClass('nav-toggle-white')
    ...
    else
    {
      ...
      $('#nav-toggle span').removeClass('nav-toggle-white')
      $('#header .hamburger').css({'background-position':'0 0'});
      //$('#cat-cont').css({'position': 'relative', 'top': 0});
    }
    
    var targetHeight = $('#bg-video').outerHeight();
	var scrollPercent = (targetHeight - window.scrollY) / targetHeight;
	if(scrollPercent >= 0){
		$('#bg-video').css('opacity', scrollPercent);
	}
